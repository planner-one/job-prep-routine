...
test('자유 학습은 강의 목록에서 시작하고 날짜별 강제 과제나 대형 배너를 두지 않는다', () => {
  ...
  assert.match(html, /learning-app\.js\?v=12/);
...
test('초기 데이터 로딩과 모듈 실패를 빈 화면 대신 안내한다', () => {
  ...
  assert.match(html, /import\('\.\/src\/learning-app\.js\?v=12'\)\.catch/);
  assert.match(html, /role="alert"/);
  assert.match(html, /다시 불러오기/);
  assert.match(app, /learning-core\.js\?v=12/);
  assert.match(app, /learning-curriculum\.js\?v=12/);
});
